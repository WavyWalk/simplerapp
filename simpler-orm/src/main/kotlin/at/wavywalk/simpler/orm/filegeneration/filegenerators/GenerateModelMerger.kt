...
class GenerateModelMerger(val modelDataModel: ModelDataModel) {
    fun run(){
        val file: File = GeneratedFileFactory.createDefault(
            packageName = modelDataModel.packagesBean.baseGenerated,
            fileName = "${modelDataModel.modelClass}Merger"
        )

        val templateName: String = TemplateFilePathsConfig.modelMerger
